...
test("launch: decoration never wears a verdict colour", () => {
  withEnv({ FORCE_COLOR: "1", CIRVIX_TRUECOLOR: "1", NO_COLOR: undefined, TERM: "xterm-256color" }, () => {
...
test("launch: NO_COLOR degrades to plain text rather than escape garbage", () => {
  withEnv({ NO_COLOR: "1", FORCE_COLOR: undefined, CIRVIX_TRUECOLOR: undefined }, () => {
    const { frames, settle } = launchFrames();
...
test("launch: a 16-colour terminal gets the accent role, never a 24-bit sequence", () => {
  withEnv({ FORCE_COLOR: "1", CIRVIX_TRUECOLOR: "16", NO_COLOR: undefined, COLORTERM: undefined }, () => {
...
test("launch: an enabled run draws, then lands on the plate and gives the cursor back", async () => {
  ...
  const result = await playLaunch({ stdout, stdin, pace: 1 });
...
test("launch: the banner is skipped for json, CI, a pipe, and a narrow terminal", async () => {
  const cases = [
    ["--json", { json: true }, ttyStream(), null],
    ["CI", {}, ttyStream(), { CI: "1", FORCE_COLOR: undefined }],
    ["a pipe", {}, ttyStream({ isTTY: false }), null],
    ["a narrow terminal", {}, ttyStream({ columns: LAUNCH_WIDTH }), null],
  ];

  for (const [label, options, stdout, env] of cases) {
    const run = async () => {

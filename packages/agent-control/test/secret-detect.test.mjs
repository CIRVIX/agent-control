...
test("substitution replaces the handle with the material", async () => {
  ...
  assert.equal(result.value.headers.authorization, "Bearer rk_" + "live_KKKKKKKKKKKK0123456789");
});
...
test("one unresolvable handle refuses the whole call", async () => {
  const vault = new Vault();
  const good = vault.issue("A", "rk_" + "live_HHHHHHHHHHHHHHHHHHHH");
  const result = await vault.substitute(
...
test("maxUses is enforced", async () => {
  const vault = new Vault();
  const handle = vault.issue("KEY", "rk_" + "live_KKKKKKKKKKKK0123456789", { maxUses: 1 });
...
test("an expired handle does not resolve", async () => {
  const vault = new Vault();
  const handle = vault.issue("KEY", "rk_" + "live_KKKKKKKKKKKK0123456789", { ttlSeconds: -1 });
...
test("the return path swaps material back for its handle", () => {
  const vault = new Vault();
  const handle = vault.issue("KEY", "rk_" + "live_KKKKKKKKKKKK0123456789");
  const result = vault.redact({ body: "leaked rk_" + "live_KKKKKKKKKKKK0123456789 here" });
  assert.ok(result.payload.body.includes(handle));
  assert.ok(!result.payload.body.includes("rk_" + "live_KKKKKKKKKKKK0123456789"));
});
...
test("forget drops everything", () => {
  const vault = new Vault();
  vault.issue("KEY", "rk_" + "live_KKKKKKKKKKKK0123456789");
...
test("a sealed vault round-trips, and a wrong passphrase fails", async () => {
  ...
  original.issue("KEY", "rk_" + "live_KKKKKKKKKKKK0123456789", { destinations: ["api.stripe.com"] });
